function [logText] = twotoneAnalyzeALEX(fileName,twotoneData);
% function [logText] = twotoneAnalyzeALEX(fileName,twotoneData);
% 
...
if strcmp(extension,'.fits')
  ImageInfo =  fitsinfo(fileName);%fits_read_header(fileName);
...
try
  ...
  % run autodetection
  [twotoneData, clusteredData, logTextAutoDet] = autoDetectALEX(fileName,twotoneData);
...
if autoDetectIsOK == true
  try
    ...
    [twotoneData] = extractDataALEX(fileName,twotoneData,clusteredData);
