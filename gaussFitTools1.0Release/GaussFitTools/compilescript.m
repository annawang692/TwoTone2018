...
elseif strcmp(ostype,'PCWIN64')%linux 64bit install 
    binFolder = 'GaussFitTools_bin_Win64';
  for i = 1:numel(file)
    txt = ['-l','levmar',' -L', 'GaussFitTools_bin_Win64 ', file{i} ]  
    mex  file{i}, '-llevmar2',' -LGaussFitTools_bin_Win64'
  end
  outfileType = 'mexw64';
  movefile(['*',outfileType,'*'],binFolder);
  copyfile(['*.m'],binFolder);
